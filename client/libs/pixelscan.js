...
class UpdateTexture extends PIXI.BaseTexture {
    ...
    constructor(width, height, options) {
        super(new PIXI.BufferResource(new Uint8Array(width * height * 4), {width: width, height: height}), options);
    }
    ...
    writePixel(x, y, r, g, b, a) {
        ...
        this.resource.data[index * 4] = r;
        this.resource.data[index * 4 + 1] = g;
        this.resource.data[index * 4 + 2] = b;
        this.resource.data[index * 4 + 3] = a;
    ...
    getAlpha(x, y) {
        const index = x + y * this.width;
        return this.resource.data[index * 4 + 3];
    }
}
class WaterShader extends PIXI.Filter {
    ...
    void main(void) {
        ...
        waterTotal = min(pow(waterTotal, 6.0), 1.0);
    ...
    getBackgroundColor() {
        return [this.uniforms.uBackgroundColor[0], this.uniforms.uBackgroundColor[1], this.uniforms.uBackgroundColor[2]];
    }

    setBackgroundColor(color) {
        this.uniforms.uBackgroundColor[0] = color[0];
        this.uniforms.uBackgroundColor[1] = color[1];
        this.uniforms.uBackgroundColor[2] = color[2];
    }
}
class Input {
    static KEY_0 = '0';
...
window.addEventListener('load', () => {
    ...
    document.addEventListener('wheel', event => {
        Input.scrollDelta = event.deltaY / 100;
...
class BinaryHelper {
    ...
    static readByte(bytes, index) {
        return bytes[index];
    }

    static readFloat(bytes, index) {
        if (LE) {
            BinaryHelper._byteBuffer[0] = bytes[index + 0];
            BinaryHelper._byteBuffer[1] = bytes[index + 1];
            BinaryHelper._byteBuffer[2] = bytes[index + 2];
            BinaryHelper._byteBuffer[3] = bytes[index + 3];
        } else {
            BinaryHelper._byteBuffer[0] = bytes[index + 3];
            BinaryHelper._byteBuffer[1] = bytes[index + 2];
            BinaryHelper._byteBuffer[2] = bytes[index + 1];
            BinaryHelper._byteBuffer[3] = bytes[index + 0];
        }

        return BinaryHelper._floatBuffer[0];
    }

    static readInt(bytes, index) {
        if (LE) {
            BinaryHelper._byteBuffer[0] = bytes[index + 0];
            BinaryHelper._byteBuffer[1] = bytes[index + 1];
            BinaryHelper._byteBuffer[2] = bytes[index + 2];
    ...
    static readUnsignedInt(bytes, index) {
        if (LE) {
            BinaryHelper._byteBuffer[0] = bytes[index + 0];
            BinaryHelper._byteBuffer[1] = bytes[index + 1];
            BinaryHelper._byteBuffer[2] = bytes[index + 2];
    ...
    static readUnsignedLong(bytes, index) {
        if (LE) {
            BinaryHelper._byteBuffer[0] = bytes[index + 0];
            BinaryHelper._byteBuffer[1] = bytes[index + 1];
            BinaryHelper._byteBuffer[2] = bytes[index + 2];
            ...
            BinaryHelper._byteBuffer[0] = bytes[index + 7];
            BinaryHelper._byteBuffer[1] = bytes[index + 6];
            BinaryHelper._byteBuffer[2] = bytes[index + 5];
            BinaryHelper._byteBuffer[3] = bytes[index + 4];
        }
        const int1 = BinaryHelper._uintBuffer[0];

        if (LE) {
            BinaryHelper._byteBuffer[0] = bytes[index + 4];
            BinaryHelper._byteBuffer[1] = bytes[index + 5];
            BinaryHelper._byteBuffer[2] = bytes[index + 6];
            BinaryHelper._byteBuffer[3] = bytes[index + 7];
        } else {
            BinaryHelper._byteBuffer[0] = bytes[index + 3];
            BinaryHelper._byteBuffer[1] = bytes[index + 2];
            BinaryHelper._byteBuffer[2] = bytes[index + 1];
    ...
    static writeByte(byte, bytes, index) {
        bytes[index] = byte;
        return index + 1;
    };
...
const UpdateTexture = PixelScan.UpdateTexture;
